...
collector health and pipeline health and exposes this data via its health check services.

Below is a table enumerating component statuses and their meanings. These will be mapped to
appropriate status codes for the protocol in use.
...
this configuration provided is unfiltered and may contain sensitive information. As such, the
configuration is disabled by default. Enabled it using the `http.config.enabled` setting. By
...
#### Mapping of ComponentStatus to healthpb.ServingStatus
...
return a HealthCheckResponse with the serving status as indicated by the table above.
...
the service is unknown, a response with HealthCheckResponse_SERVICE_UNKNOWN will be sent. The stream
will remain open, and if and when the service starts reporting, its status will begin streaming.
